...
def generate_response(query, fact_extract):
    ...
    combined_context = (
                        f"User Query:\n {query}"
                        f"\n Facts:\n {fact_extract}"
                        f"\n\n Corpus from Knowledge Graph (A) and Referenced Passages (B): \n"
                        f"A. Context from the Knowledge Graph - \n"
                        f"1. Cosine similarity vector-searched nodes and edges from the Knowledge Graph:\n{graph_info}\n\n"
                        f"2. Contextual neighbors of the previous nodes:\n{contextual_nodes_info}\n\n"
                        f"3. Shortest path through the graph that connects all vector-searched nodes:\n{tsp_path_str}\n\n"
                        f"4. Topic clusters in the graph based on the query:\n{topic_triplets_str}\n\n"
                        f"5. Additional top_nodes for the query:\n{top_nodes_triplets_str}\n\n"
                        f"6. Contextual neighbors of the additional top_nodes:\n {contextual_nodes_info_top_nodes}\n\n"
                        f"B. Context from referenced passages in the corpus:\n{references_query}\n{references}\n"
                    )
    ...
    full_interaction = (
                        ...
                        f"\n\n Corpus from Knowledge Graph (A) and Referenced Passages from the Base Corpus (B): \n"
                        f"A. Context from the Knowledge Graph - \n"
                        f"1. Cosine similarity vector-searched nodes and edges from the Knowledge Graph:\n{graph_info}\n\n"
                        f"2. Contextual neighbors of the previous nodes:\n{contextual_nodes_info}\n\n"
                        f"3. Shortest path through the graph that connects all vector-searched nodes:\n{tsp_path_str}\n\n"
...
if os.path.exists(text_vectorstore_path):
    ...
    parent_texts = semantic_parent_splitting("./data_input")
